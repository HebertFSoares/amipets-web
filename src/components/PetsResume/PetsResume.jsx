import { getPetsResumeData } from "@/utils/getPetsResumeData";
...
export function PetsResume() {
    ...
    getPetsResumeData(petsCached)

    let resume;

    if (petsCached) {
        resume = getPetsResumeData(petsCached);
    }

    return (
        <div className={`flex flex-col shadow-center-sm px-8 py-7 basis-40 min-w-44`}  >
            ...
            <div className="flex justify-between items-center mb-2">
                ...
                <span className="font-rubik text-sm font-medium text-lilac">{resume && resume.livres}</span>
            </div>
            <div className="flex justify-between items-center mb-4">
                <span className="font-rubik text-sm font-medium">Adotados</span>
                <span className="font-rubik text-sm font-medium text-orange">{resume && resume.adotados}</span>
            </div>
            <div className=" border-b border-b-[#EAEAEA] mb-4"></div>
            <div className="flex justify-between">
                <span className="font-rubik text-sm font-bold">Em análise</span>
                <span className="font-rubik text-sm font-medium text-sky-500">{resume && resume.emAnalise}</span>
            </div>
        </div>
    );
}